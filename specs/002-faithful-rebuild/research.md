...
**Decision**: V2's `src/mcp-local/` uses `@modelcontextprotocol/sdk` (the official Node SDK) with **in-process** transport (not stdio, not HTTP) so the engine's adapter can talk to it without spawning a subprocess. The 9 inherited tools (7 from 001's action set + `publish_post` + `terminate`) are registered on this server. The engine's `addAdapter` takes the in-process server reference at boot.

**Rationale**: The MCP SDK supports multiple transports; in-process avoids the IPC overhead and process management of stdio. The local module is V2-internal infrastructure (FR-200a) — no need to expose it externally. Same SDK as the engine's other adapter consumers, so the engine's discovery code path treats it identically to a remote MCP server, which is exactly FR-200's "single intake" requirement.
