...
- Q: How are operator-only write/admin endpoints (pause / resume / infrastructure_note) authenticated? → A: Option A. Bearer-token auth against the `OPERATOR_AUTH_TOKEN` env var (inherited from 001) on all `/operator/*` write/admin endpoints. Read-only endpoints (`/transcript`, `/memory`, `/data`, `/blog`, `/identity`, `/goals`, `/drives`, `/watchdog`, `/coherence`, `/control/*`, `/scores`) remain publicly readable per Principle III. The `/scores` endpoint adds an agent-egress filter on top — requests whose source matches the agent's egress are rejected (so the agent cannot read its own scores). Public observers see scores; only the agent process is blocked. Minimum surface that satisfies Principle IX without compromising Principle III. **Updated 2026-05-08**: bearer-token requirement on `/scores` was dropped after operator decision that public score visibility (Principle III) outweighs duplicating the agent-block already provided by the egress filter (Principle IX). The egress filter is now the sole gate; previously this clarification mandated bearer + egress.
...
- **FR-039**: Dashboard endpoint `/scores` returns the rater's good/evil scores per summary. Publicly readable per Principle III. The agent has NO read path: enforcement is the agent-egress filter specified in FR-134 (sole gate). **Updated 2026-05-08**: the bearer-token check previously mandated by this FR was dropped — public observers (and the public dashboard frontend) need to see scores, and the egress filter alone blocks the agent. Bearer would have only blocked observers without affecting the agent, which inverted the intent.
...
- **FR-134**: The `/scores` endpoint MUST apply an agent-egress filter as the sole gate: requests whose source IP / network identity matches the agent process MUST be rejected with HTTP 403 (`code: forbidden_egress`). FR-064 holds: the agent has no read path to its own scores. **Updated 2026-05-08**: previously this filter was layered on top of a bearer-token check; bearer was dropped (see FR-039 update) because it blocked public observers without affecting the agent (the egress filter handles the agent). Egress is now the only gate, and it is the gate that actually enforces the contamination boundary.
